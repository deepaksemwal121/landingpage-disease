...
const BookAppointment = () => {
    ...
    const bookHandler = async (e) => {
        ...
        try {
            const res =  await axios.post("http://3.108.40.250/api/v1/lead/",data);
    ...
    <div className='md:px-32 md:py-16 px-8 py-8 md:flex w-full'>
        <div className='space-y-10 md:w-1/2'>
            ...
            <form className='flex flex-col space-y-4 px-4' onSubmit={bookHandler}>
                ...
                <input id='fullname' type="text" className='bg-white rounded-sm border-[2px] p-2' value={details.name} onChange={(e)=> setDetails({...details, name:e.target.value})} />
                </>
                <>
                <label htmlFor='email'>Your Email</label>
                <input id='email' type="email" className='bg-white rounded-sm border-[2px] p-2' value={details.email} onChange={(e)=> setDetails({...details, email:e.target.value})} />
                </>
                <>
                <label htmlFor='phone'>Your Phone</label>
                <input id='phone' type="text" className='bg-white rounded-sm border-[2px] p-2' value={details.phone} onChange={(e)=> setDetails({...details, phone:e.target.value})} />
                </>
                <>
